...
- 如果发现文件当前内容相较于我上次修改后的版本已经发生变化，不要还原成上次版本，应以当前文件内容为基础继续修改
- 中文文件只用 `apply_patch` 修改，不再使用可能触发编码转换的写回命令
- 每次修改完成后，如当前修改文件属于 Prettier 可处理的文本文件类型，先直接对该单文件执行 `prettier --write "具体文件路径"`，不得通过 `npm run prettier`、`pnpm prettier` 或其他 `package.json` 脚本方式触发
